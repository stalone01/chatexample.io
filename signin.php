...
if(isset( $_POST['signin'] )) {
	...
	if(BDD::_query('SELECT email FROM users WHERE email=:email', [ 'email' => $email ])) {
		if(password_verify($password, BDD::_query('SELECT password FROM users WHERE email=:email', [ 'email' => $email ])[0]['password'])) {
			$cstrong = TRUE;
			$token = bin2hex(openssl_random_pseudo_bytes(64, $cstrong));
			$user_id = BDD::_query('SELECT id FROM users WHERE email=:email', [ 'email' => $email ])[0]['id'];

			BDD::_query('INSERT INTO login_tokens (user_id, token) VALUES(:user_id, :token)', [ 
				'user_id' 	=> $user_id, 
				'token' 	=> sha1($token) 
			]);
