...
if(isset($_POST['signup'])) {
	...
	if(!BDD::_query('SELECT id FROM users WHERE username=:username', [ 'username' => $username ])) {
		// Is email already taken?
		if(!BDD::_query('SELECT id FROM users WHERE email=:email', [ 'email' => $email ])) {
				...
				if(strlen($password) >= 3 && strlen($password) < 30) {
					...
					if(preg_match('/[a-zA-Z0-9_]+/', $username)) {
						...
						if(filter_var($email, FILTER_VALIDATE_EMAIL)) {
							...
							BDD::_query('INSERT INTO users (username, password, email) VALUES (:username, :password, :email)', [
								'username'  => $username, 
