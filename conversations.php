...
$users_id = Login::isLogged();
$username = BDD::_query('SELECT username FROM users WHERE id=:user_id', ['user_id'=> $user_id ])[0]['username'];

//si l'utilisateur veut deconnecter
if(isset($_POST['logout'])){
    // supprimer la login data de la bdd
    if(BDD::_query('SELECT id FROM login_tokens WHERE user_id=:user_id', ['user_id'=>$user_id])){
        BDD::_query('DELETE FROM login_tokens WHERE user_id=:user_id', ['user_id'=>$user_id]);
    }
    //supprimer la cookies login
...
//si l'utilisateur veut changer de photo de profile
if(isset($_POST['avatar-file'])){
    ...
    if(!Image::$error){
        ...
        if(BDD::_query('SELECT img_path FROM imgs WHERE user_id=:user_id', [ 'user_id' => $user_id ])) {
			BDD::_query('UPDATE imgs SET img_path=:img_path WHERE user_id=:user_id', [ 'img_path' => $img_path, 'user_id' => $user_id ]);
		} else {
			BDD::_query('INSERT INTO imgs (img_path, user_id) VALUES (:img_path, :user_id)', [ 'img_path' => $img_path, 'user_id' => $user_id ]);
		}
	...
	<div class="row">
		...
		<section class="col-md-4 conversations-section">
			<ul class="user-list">
				<?php
					// List of users who wrote you or you wrote them.
					if(BDD::_query('SELECT users.username FROM users, messages WHERE messages.receiver = users.id OR messages.sender = users.id AND users.id = :user_id', [ 'user_id' => $user_id ])) {
						$usernames = BDD::_query('SELECT * FROM messages, users WHERE (messages.receiver = :user_id OR messages.sender = :user_id) AND (messages.receiver = users.id OR messages.sender = users.id) GROUP BY users.id', [ 'user_id' => Login::isLogged() ]);
